...
- **专属 Profile 隔离与依赖实路径**：青梧启动引擎时采用专属的 `qingwu` Profile（位于 `~/.dsh/profiles/qingwu`），与官方终端 Web 版（`profiles/web`）及官方桌面版（`profiles/desktop`）配置和插件完全物理隔离，首次启动自动初始化；dsh profile 中的插件 junction 需要真实的 `node_modules` 路径，不能把整棵依赖重新塞进 `app.asar`；当前 `asarUnpack: ["node_modules/**"]` 是运行约束。打包前钩子必须保证随包 Node 就绪。
