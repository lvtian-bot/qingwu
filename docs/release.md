...
1. 确认本次版本范围：
   - 先说明拟用版本号、影响范围与发布后果，并取得明确同意；未确认前不得修改版本字段或创建标签。版本号按最后一位递增、满 9 进位到中间位，规则见 `AGENTS.md`。
...
2. 运行 `npm run check`，确认类型检查、行为与契约测试、构建全部通过；按 [维护导航](maintenance.md) 验证本次涉及的桌面交互。引擎升级必须验证双界面与命令执行，仅编译通过不足以证明可发布。
