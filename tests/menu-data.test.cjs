...
test('菜单数据：各菜单项分类与结构完备性（对齐 ChatGPT）', () => {
  const fileItems = getMenuItems('文件');
  ...
  const editItems = getMenuItems('编辑');
  ...
  const viewItems = getMenuItems('视图');
  ...
  assert.equal(viewItems.some((item) => item.id === 'switchUiMode'), false);

  const helpItems = getMenuItems('帮助');
