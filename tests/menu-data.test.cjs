...
test('菜单数据：各菜单项分类与结构完备性（对齐 ChatGPT）', () => {
  const fileItems = getMenuItems('文件', { closeToTray: false, uiMode: 'native' });
  ...
  const editItems = getMenuItems('编辑', { closeToTray: false, uiMode: 'native' });
  ...
  const viewItems = getMenuItems('视图', { closeToTray: false, uiMode: 'native' });
  ...
  const helpItems = getMenuItems('帮助', { closeToTray: false, uiMode: 'native' });
  assert.ok(helpItems.some((item) => item.id === 'checkForUpdates'));
  assert.ok(helpItems.some((item) => item.id === 'about'));
});

test('菜单数据：closeToTray 勾选状态与 uiMode 文案动态联动', () => {
  const fileUnchecked = getMenuItems('文件', { closeToTray: false, uiMode: 'native' });
  const trayItemUnchecked = fileUnchecked.find((item) => item.id === 'toggleCloseToTray');
  assert.equal(trayItemUnchecked?.checked, false);

  const fileChecked = getMenuItems('文件', { closeToTray: true, uiMode: 'native' });
  const trayItemChecked = fileChecked.find((item) => item.id === 'toggleCloseToTray');
  assert.equal(trayItemChecked?.checked, true);

  const viewNative = getMenuItems('视图', { closeToTray: false, uiMode: 'native' });
  const switchNative = viewNative.find((item) => item.id === 'switchUiMode');
  assert.equal(switchNative?.label, '切换到 DeepSeek 界面');

  const viewOfficial = getMenuItems('视图', { closeToTray: false, uiMode: 'official' });
